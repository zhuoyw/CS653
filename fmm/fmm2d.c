...
/*----------------------------------------------------------------------------*/
  ...
  printf("===== FMM & direct CPU times = %e %e =====\n",
    (t2-t1)/(double)CLOCKS_PER_SEC,(t3-t2)/(double)CLOCKS_PER_SEC);
  printf("===== FMM & direct Gflop = %e %e =====\n",
    ffmm/1e9,fdirect/1e9);
  printf("===== FMM & direct Gflop/s = %e %e =====\n",
    ffmm/((t2-t1)/(double)CLOCKS_PER_SEC)/1e9,fdirect/((t3-t2)/(double)CLOCKS_PER_SEC)/1e9);
...
------------------------------------------------------------------------------*/
  ...
  eng_direct *= 0.5;
#ifdef COUNT
  fdirect += (double)(Npar)*(double)(Npar)/2*(2*3+10+10+10+4);
  fdirect += Npar*2;
#endif
}
