...
int main(int argc, char **argv) {
  ...
  nx = NX/2;
  dim = nx + 2;
  cudaMalloc(&dev_psi, 2*dim);
  cudaMalloc(&dev_wrk, 2*dim);
  cudaMalloc(&dev_al, 2*2);
  cudaMalloc(&dev_bux, 2*2*dim);
  cudaMalloc(&dev_blx, 2*2*dim);
  cudaMalloc(&dev_u, 2*dim);

  cudaMemcpy(psi + nx*mytid, dev_psi, 2*dim, cudaMemcpyHostToDevice);
  cudaMemcpy(wrk + nx*mytid, dev_wrk, 2*dim, cudaMemcpyHostToDevice);
  cudaMemcpy(al,             dev_al,  2*2,   cudaMemcpyHostToDevice);
  cudaMemcpy(bux + nx*mytid, dev_bux, 2*dim, cudaMemcpyHostToDevice);
  cudaMemcpy(bux + nx*mytid + 2*(NX+2), dev_bux+2*dim, 2*dim, cudaMemcpyHostToDevice);
  cudaMemcpy(blx + nx*mytid, dev_blx, 2*dim, cudaMemcpyHostToDevice);
  cudaMemcpy(blx + nx*mytid + 2*(NX+2), dev_blx+2*dim, 2*dim, cudaMemcpyHostToDevice);
  cudaMemcpy(u   + nx*mytid, dev_u,   2*dim, cudaMemcpyHostToDevice);

	for (step=1; step<=NSTEP; step++) {
		single_step(dev_psi, dev_wrk, dev_al, dev_bux, dev_blx, dev_u); /* Time propagation for one step, DT */
    #pragma omp master
		if (step%NECAL==0) {
			calc_energy();
      if (myid == 0) {
...
/*----------------------------------------------------------------------------*/
void single_step(double* dev_psi, double* dev_wrk, double* dev_al, double* dev_bux, double* dev_blx, double*dev_u) {
/*------------------------------------------------------------------------------
	Propagates the electron wave function for a unit time step, DT.
------------------------------------------------------------------------------*/
	pot_prop(dev_psi, dev_u);  /* half step potential propagation */

	kin_prop(0, dev_psi, dev_wrk, dev_al, dev_bux, dev_blx); /* half step kinetic propagation   */
	kin_prop(1, dev_psi, dev_wrk, dev_al, dev_bux, dev_blx); /* full                            */
	kin_prop(0, dev_psi, dev_wrk, dev_al, dev_bux, dev_blx); /* half                            */
...
/*----------------------------------------------------------------------------*/
void pot_prop(double* dev_psi, double* dev_u) {
/*------------------------------------------------------------------------------
...
------------------------------------------------------------------------------*/
  ...
  */
  ...
  cudaMemcpy(dev_psi, psi + nx*mytid, 2*dim, cudaMemcpyDeviceToHost);
  cudaMemcpy(dev_u,   u   + nx*mytid,   2*dim, cudaMemcpyDeviceToHost);
  /*
	for (sx=1; sx<=NX; sx++) {
...
__global__ void gpu_kin_prop(int t, double* al, double* psi, double* wrk, int nx) {
  int idx = blockIdx.x*blockDim.x+threadIdx.x;
  if (idx >= 1 && idx <= nx) {
    t = 2*t;
    idx = 2*idx;
    double wr = al[t] * psi[idx] - al[t+1] * psi[idx+1];
    double wi = al[t] * psi[idx+1] - al[t+1] * psi[idx];
    wr += blx[t*dim+idx] * psi[idx-2] - blx[t*dim+idx+1] * psi[idx-1];
    wi += blx[t*dim+idx] * psi[idx-1] + blx[t*dim+idx+1] * psi[idx-2];
    wr += bux[t*dim+idx] * psi[idx+2] - bux[t*dim+idx+1] * psi[idx+3];
    wr += bux[t*dim+idx] * psi[idx+3] + bux[t*dim+idx+1] * psi[idx+2];
    wrk[idx] = wr;
    wrk[idx+1] = wi;
}

/*----------------------------------------------------------------------------*/
void kin_prop(int t, double* dev_psi, double* dev_wrk, double* dev_al, double* dev_bux, double* dev_blx) {
/*------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------*/
  ...
  */
  ...
  dim3 dimGrid((dim+BLOCKDIM-1)/BLOCKDIM,1,1); // Grid dimensions (only use 1D)
  dim3 dimBlock(BLOCKDIM,1,1); // Block dimensions (only use 1D)
  gpu_kin_prop<<<dimGrid,dimBlock>>>(t, dev_al, dev_psi, dev_srk, nx);

  cudaMemcpy(dev_wrk, wrk + nx*mytid, 2*dim, cudaMemcpyDeviceToHost);
